...
def etl_pisa2003_student_final_notas():
    # Conectar ao MongoDB
    ...
    print("⏳ Lendo o arquivo corrigido para notas...")
    df = pd.read_csv(
        ARQUIVO_STUDENT,
        sep=r"\s+",
        engine="python",
    ...
    # Extração corrigida dos campos
    df_extracao = pd.DataFrame()
    df_extracao["CNT"] = df[0].str.slice(3, 6)
    df_extracao["PV1READ"] = pd.to_numeric(df[91], errors='coerce')
    df_extracao["PV1MATH"] = pd.to_numeric(df[115], errors='coerce')
    df_extracao["PV1SCIE"] = pd.to_numeric(df[139], errors='coerce')
...
if __name__ == "__main__":
    etl_pisa2003_student_final_notas()
